...
// operator can see order-pipeline events (failed / dead_letter) on
// their phone without it being a wake-up alarm. Runs in its own
// goroutine with a 5-second timeout so it never blocks the batch
// loop or holds the SYSPRO session mutex.
//
